            ...
            <div className="flex flex-row justify-between">
                <div className="flex flex-row items-start">
                    <button className=" mt-2 mr-2 min-w-fit">
                        <img src={`https://picsum.photos/32?random=${Math.floor(Math.random() * 1000)}`} alt="" className="w-8 h-8 rounded-full" />
                    </button>

                    <div>
                        <p className="font-semibold text-sm text-white pt-2 pb-1 tracking-wide">{title}</p>
                        <p className="font-thin text-xs text-neutral-400 py-1">
                            {creator.name} &nbsp;
                            {creator.verified ? <img src="/icons/verified.svg" alt="" className="w-3 inline" /> : ""}
                        </p>
