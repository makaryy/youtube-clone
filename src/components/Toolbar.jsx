        ...
        <div className="py-1 bg-menu flex flex-row w-full h-fit items-center border-b border-neutral-700 max-h-min opacity-[.98]">
            ...
            <div className="flex flex-row h-10 min-w-min max-w-2xl grow-4">
                <form onSubmit={(e) => handleSubmit(e)} className="grow flex flex-row">
                    ...
                    <div className="grow bg-search-bar border-search-button border rounded-l-sm text-white flex flex-row items-center">
                        {showIcon && <img src="/icons/search.svg" alt="" className="w-5 h-5 mx-2" />}
                        <input
                            type="text"
                            name="search"
                            value={search}
                            ...
                            placeholder="Search"
                            className={showIcon ? "bg-search-bar grow px-2 outline-none" : "bg-search-bar grow px-2 outline-none"}
                            onFocus={() => setShowIcon(true)}
                            onBlur={() => setShowIcon(false)}
                        />
                    </div>

                    <button type="submit" className="rounded-r-sm w-16 flex justify-center items-center bg-search-button">
                        <img src="/icons/search.svg" alt="" />
                    </button>
                    <ToolbarButton src="/icons/microphone.svg" style={{ backgroundColor: "rgb(18,18,18)" }} />
                </form>
            </div>
            <div className="flex justify-end items-center grow">
                ...
                <button className="px-4 py-2">
                    <img src={`https://picsum.photos/48?random=${Math.floor(Math.random() * 1000)}`} alt="" className="w-8 h-8 mx-2 rounded-full" />
                </button>
            </div>
        </div>
    );
